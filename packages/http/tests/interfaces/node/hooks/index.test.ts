...
describe("makeNodeHook", () => {
	...
	describe("sendResponse", () => {
		...
		it("writes file body", async() => {
			...
			await hooks.sendResponse({
				request,
				currentResponse: { body: DSFile.createFileInterface(DCommon.cast("test.txt")) },
				exit: exitHookFunction,
			} as any);
			...
			it("send events", async() => {
				...
				request.raw.response.write = vi.fn()
					.mockImplementationOnce(
						(data) => {
							write(data);
							void DCommon.timeout().then(() => request.raw.response.emit("drain"));
							return false;
						},
					)
					.mockImplementationOnce(
	...
	describe("afterSendResponse", () => {
		it("delete filesAttache", async() => {
			...
			request.filesAttache = [DCommon.cast("test.txt"), DCommon.cast("tot.png")];
