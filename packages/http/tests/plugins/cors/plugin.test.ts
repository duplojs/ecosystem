import { createHub, HookResponse, launchHookBeforeBuildRoute, launchHookServer, Request, Response, type RequestInitializationData, ResponseContract, useRouteBuilder, type ResponseCode } from "@core";
...
describe("cors plugin", () => {
	it("when building routes, injects cors options route", async() => {
		...
		const optionsRoute = DArray.find(routes, (route) => route.definition.method === "OPTIONS");
	...
	it("when allowOrigin is a function", async() => {
		const hub = createHub({ environment: "DEV" })
			.plug(
				corsPlugin({
					allowOrigin: DCommon.equal("https://example.com"),
				}),
			)
			.register(basicRoutes);
	...
	it("i'm out of ideas for 'it message'", async() => {
		...
		const optionsRoute = DArray.find(
			DArray.from(hub.routes),
