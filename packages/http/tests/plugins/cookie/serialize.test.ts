...
describe("defaultSerializer", () => {
	...
	it("serializes expires with the HTTP-date format", () => {
		const result = defaultSerializer(
			...
			"value",
			{
				expires: DChrono.createDateOrThrow("date1704164645000+"),
			},
		);
	...
	it("serializes expireIn from a DChrono.TheTime value", () => {
		vi.useFakeTimers();
		...
		try {
			...
			const result = defaultSerializer(
				...
				"value",
				{
					expireIn: DChrono.createTime(1, "hour"),
				},
			);
	...
	it("rejects expires and expireIn together", () => {
		expect(
			() => defaultSerializer(
				...
				"value",
				{
					expires: DChrono.createDate("2024-01-02"),
					expireIn: DChrono.createTime(1, "hour"),
				} as never,
			),
