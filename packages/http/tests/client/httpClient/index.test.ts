...
import { type AllClientResponse, autoCreateCacheKey, createHttpClient, type Hooks, httpClientKind, type PromiseRequestParams } from "@client";
...
describe("httpClient", () => {
	...
	describe("hooks", () => {
		...
		const requestHook: Hooks["request"][number] = vi.fn(
			DCommon.forward<PromiseRequestParams<Record<string, unknown>>>,
		);
		const responseHook: Hooks["response"][number] = vi.fn(
			DCommon.forward<AllClientResponse<Record<string, unknown>>>,
		);
