import { createKind, createHookHubLifeCycle, createHub, launchHookBeforeBuildRoute, launchHookServer, launchHookServerError, serverErrorExitHookFunction, serverErrorNextHookFunction } from "@core";
...
describe("hub hooks", () => {
	...
	it("launchHookServer", async() => {
		...
		const fakeHook2 = vi.fn().mockResolvedValue(createKind("server-hook-next").setTo({}, null));
