...
export class SerializeCookieError extends DKind.parentClass(
	createKind("serialize-cookie-error"),
	Error,
) {
	public constructor(
		message: string,
...
export function defaultSerializer(
	...
	if (params?.expireIn !== undefined) {
		setCookie += `; Expires=${DChrono.addTime(DChrono.now(), params.expireIn).toUTCString()}`;
	}
