...
> extends Promise<
	...
	public constructor(
		...
		super(
			(resolve) => void DEither.asyncSafeCallback(
				DEither.rightAsyncPipe(
					...
					async(response) => {
						if (params.disabledPredicateMode === false && response.predicted === false) {
							...
							return DEither.right("response", response);
						}

						if (DString.startsWith(response.code, "1")) {
							await launchResponseTypeHook(
								params.hooks.informationalResponseType,
								this.hooks.informationalResponseType ?? [],
								response,
							);
						} else if (DString.startsWith(response.code, "2")) {
							await launchResponseTypeHook(
						...
						} else if (DString.startsWith(response.code, "3")) {
							await launchResponseTypeHook(
								params.hooks.redirectionResponseType,
								this.hooks.redirectionResponseType ?? [],
								response,
							);
						} else if (DString.startsWith(response.code, "4")) {
							await launchResponseTypeHook(
						...
						} else if (DString.startsWith(response.code, "5")) {
							await launchResponseTypeHook(
				...
				.then(async(result): Promise<MaybeResponse> => {
					if (DEither.hasInformation(result, "safe-callback-error")) {
						const error = DEither.unwrapLeft(result);
	...
	public addRequestInterceptor(
		...
		) => DCommon.MaybePromise<GenericClientResponse["requestParams"]>,
	) {
		this.hooks.request ??= [];
	...
	public addResponseInterceptor(
		callback: (response: GenericClientResponse) => DCommon.MaybePromise<GenericClientResponse>,
	) {
		this.hooks.response ??= [];
	...
	>(
		...
		callback: (
			response: DCommon.NeverCoalescing<
				Extract<
					GenericClientResponse,
	...
	>(
		...
		const formattedCode = DArray.coalescing(code);
	...
	public iWantInformation<
		GenericInformation extends Extract<
			...
			string
		>,
		GenericResponse extends DCommon.NeverCoalescing<
			Extract<
				GenericClientResponse,
				GenericInformation extends any
					? { information: GenericInformation }
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DArray.includes(formattedInformation, response.information)
					) {
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DArray.includes(formattedCode, response.code)
					) {
						return DEither.right(
							"response",
							response,
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DString.startsWith(response.code, "1")
					) {
						return DEither.right(
							"response",
							response,
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DString.startsWith(response.code, "2")
					) {
						return DEither.right(
							"response",
							response,
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DString.startsWith(response.code, "3")
					) {
						return DEither.right(
							"response",
							response,
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DString.startsWith(response.code, "4")
					) {
						return DEither.right(
							"response",
							response,
			...
			DEither.whenIsRight(
				(response) => {
					if (
						...
						&& DString.startsWith(response.code, "5")
					) {
						return DEither.right(
							"response",
							response,
			...
			.then(
				(maybeResponse) => {
					if (DEither.isRight(maybeResponse)) {
						return DEither.unwrapRight(maybeResponse);
					}

					throw new UnexpectedInformationResponseError(
						information,
						DEither.unwrapLeft(maybeResponse),
					);
				},
			);
	}
		...
		if (body) {
			if (!headers["content-type"]) {
				if (typeof body === "string") {
					...
					body = body.toString();
				} else if (body instanceof DCommon.TheFormData) {
