...
export const httpClientKind = createKind("http-client");
...
> = DCommon.IsEqual<GenericServerRoute, ServerRoute> extends true
	? (
		path: string,
		params?: DCommon.SimplifyTopLevel<
			Omit<
...
> extends DKind.Kind<typeof httpClientKind> {
	...
	addDefaultHeader(
		headerName: string,
		headerValue: DCommon.MayBeGetter<string | undefined | null>
	): void;

	addDefaultHeaders(
		headers: Record<
			string,
			DCommon.MayBeGetter<string | undefined | null>
		>
	): void;
	...
	const self: HttpClient = httpClientKind.setTo(
		{
			...
			addDefaultHeaders(headers) {
				for (const [header, headerValue] of DObject.entries(headers)) {
			...
			request(params) {
				const headers = DGenerator.reduce(
					defaultHeaders.entries(),
					DGenerator.reduceFrom<ClientRequestParamsHeaders>({}),
					({ item, lastValue, next }) => {
						const value = item[1]();
						if (value !== undefined) {
							lastValue[item[0]] = `${value}`;
						}
		...
		} satisfies DKind.Remove<HttpClient>,
		null,
	);
