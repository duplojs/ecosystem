		...
		(emitError, emitBeforeRetry) => {
			...
			return DCommon.pipe(
				DGenerator.asyncLoop(
					async({ next, exit }: DGenerator.LoopParams<ReadableStreamReadResult<Uint8Array>>) => {
						try {
				...
				async function *(chunkGenerator) {
					...
					for await (const chunk of chunkGenerator) {
						...
						if (DString.test(buffer, endMessageRegexp)) {
							const events = DString.split(buffer, endMessageRegexp);
							buffer = DArray.last(events);
							const restEvents = DArray.pop(events);
				...
				async function *(chunkStringGenerator) {
					for await (const stringChunk of chunkStringGenerator) {
						const eventContent = DGenerator.reduce(
							DString.extractAll(stringChunk, eventPayloadRegexp),
							DGenerator.reduceFrom<RawServerEvent>({}),
							({ lastValue, item, next }) => {
								const { field, value } = item.namedGroups as {
								...
								if (field === "data") {
									...
									const retry = DString.test(value, validRetryRegexp)
										? parseInt(value, 10)
										: undefined;
									...
									return next(
										{
											...lastValue,
											id: !DString.test(value, nullIdRegexp)
												? value
												: undefined,
										},
									);
